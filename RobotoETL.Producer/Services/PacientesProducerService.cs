...
namespace RobotoETL.Producer.Services
{
    internal class PacientesProducerService : IPacientesProducerService
    {
        ...
        private Faker<Pessoa> BuildFaker() 
        {
            ...
            fk.RuleFor(p => p.Nascimento, (f, p) => DateOnly.FromDateTime(f.Person.DateOfBirth));
            fk.RuleFor(p => p.Sexo, f => ParseGender(f.Person.Gender.ToString()));
            return fk;
        }

        private Sexo ParseGender(string gender) => (gender == "Male") ? Sexo.Masculino : Sexo.Feminino;
